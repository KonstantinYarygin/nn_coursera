...
function ret = loss(model, data, wd_coefficient)
    ...
    fprintf('classification_loss = %f, wd_loss = %f\n', classification_loss, wd_loss);
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
    ...
    loss_deriv_output_in = class_prob - data.targets;

    % loss_deriv_hidden_in = np.dot(loss_deriv_output_in, self.w2.T) * self.hidden * (1 - self.hidden)
    loss_deriv_hidden_in = transpose(model.hid_to_class) * loss_deriv_output_in .* hid_output .* (1 - hid_output);
    ...
    hid_to_class = (loss_deriv_output_in * transpose(hid_output)) + (model.hid_to_class .* wd_coefficient);
    ...
    input_to_hid = (loss_deriv_hidden_in * transpose(data.inputs)) + (model.input_to_hid .* wd_coefficient);

    ret.hid_to_class = hid_to_class .* (1 / size(data.inputs, 2));
    ret.input_to_hid = input_to_hid .* (1 / size(data.inputs, 2));
